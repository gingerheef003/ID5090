...
for s = 1:length(sig_values)
    ...
    % Calculate difference of theta to polarization parameter
    final_polarization_angle = angle(mean(exp(1i * theta)));
    theta_diff = theta - final_polarization_angle;

    % Plot histogram of theta differences
    figure;
    histogram(theta_diff, 30);
    xlabel('Difference of \theta to Polarization Parameter');
    ylabel('Number of Agents');
    title(['Histogram of \theta Differences for \sigma = ' num2str(sig)]);
end