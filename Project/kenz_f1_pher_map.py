...
MUTATION_RATE = 0.001
POPULATION_CAP = 800
K_GENERATIONS = 50  # Tunable parameter for number of generations
...
class Creature:
    ...
    def mutate(self):
        ...
        if random.random() < MUTATION_RATE/10:
            new_dna.append((
                'IH' if random.random() < 0.5 else 'HO',
                random.choice(POSSIBLE_SENSORS) if random.random() < 0.5 else random.randint(0, HIDDEN_NODES-1),
                random.randint(0, HIDDEN_NODES-1) if random.random() < 0.5 else random.choice(POSSIBLE_ACTIONS),
                random.uniform(-1, 1)
            ))
            mutations += 1
        self.dna = new_dna
...
class Simulation:
    ...
    def run_frame(self):
        ...
        for creature in self.population:
            creature.update_sensors(self.grid, self.current_time, self.pheromone_grid, self.creature_grid)
    ...
    def plot_pheromone_map(self):
        plt.figure(figsize=(10, 10))
        plt.imshow(self.pheromone_grid.T, cmap='hot', interpolation='nearest', origin='lower')
    ...
    def run_generation(self):
        ...
        all_genes = []
        for creature in self.population:
            all_genes.extend(creature.dna)
        unique_genes = set(all_genes)
        self.genetic_diversities.append(len(unique_genes))
        ...
        survival_rate = (len(survivors) / POPULATION_CAP) * 100
...
def main():
    print(f"Starting evolution simulation for {K_GENERATIONS} generations:")
    print(f"- {HIDDEN_NODES} hidden nodes")
    print(f"- Safe zone (x >= {SAFE_ZONE_START})")
    print(f"- Mutation rate: {MUTATION_RATE}")
    
    sim = Simulation()
    try:
        for _ in range(K_GENERATIONS):
            if not sim.run_generation():
                break
        ...
        # Plot survival rate
        plt.figure()
        plt.plot(sim.survival_rates, marker='o', color='b')
        plt.xlabel('Generation')
        plt.ylabel('Survival Rate (%)')
        plt.title('Survival Rate Over Generations')
        plt.grid(True)
        
        # Plot genetic diversity
        plt.figure()
        plt.plot(sim.genetic_diversities, marker='o', color='r')
        plt.xlabel('Generation')
        plt.ylabel('Genetic Diversity (Unique Genes)')
        plt.title('Genetic Diversity Over Generations')
        plt.grid(True)
        
        # Plot pheromone map
        sim.plot_pheromone_map()
