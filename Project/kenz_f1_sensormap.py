...
class Creature:
    def __init__(self, dna=None, position=None, birth_time=0):
        ...
        self.sensors = {SENSOR_MAP[s]: 0 for s in self.active_sensors}
        self.actions = {ACTION_MAP[a]: 0 for a in self.active_actions}
    ...
    def _initialize_dna(self):
        ...
        for _ in range(15):
            if random.random() < 0.5:
                dna.append((
                    'IH',
                    SENSOR_MAP[random.choice(POSSIBLE_SENSORS)],
                ...
                dna.append((
                    'HO',
                    random.randint(0, HIDDEN_NODES-1),
                    ACTION_MAP[random.choice(POSSIBLE_ACTIONS)],
                    random.uniform(-1, 1)
                ))
        return dna
    ...
    def update_sensors(self, grid, current_time, pheromone_grid, creature_grid):
        ...
        if SENSOR_MAP['Sir'] in self.sensors:
            ...
            self.sensors[SENSOR_MAP['Sir']] = np.clip((right - left)/max(1, right+left), -1, 1)
