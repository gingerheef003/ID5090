...
MUTATION_RATE = 0.01
POPULATION_CAP = 1000
SAFE_ZONE_RADIUS = GRID_SIZE // 8  # Radius of the quarter-circle safe zones
...
class Creature:
    ...
    def _generate_color(self):
        """Generate a stable color based on the creature's DNA."""
        # Create a DNA fingerprint ignoring weight values
        dna_fingerprint = tuple(sorted(
            (g[0], g[1], g[2]) if g[0] == 'IH' else (g[0], g[1], g[2])
            for g in self.dna
        ))
        hue = hash(dna_fingerprint) % 360 / 360  # Normalize hash to [0, 1]
        return plt.cm.hsv(hue)  # Use HSV colormap to generate a color
...
class Simulation:
    def __init__(self, show_graph=True):
        ...
        self.show_graph = show_graph  # New parameter to control graph display
        ...
        with open(self.csv_filename, 'w', newline='') as csvfile:
            ...
            writer.writerow(['Generation', 'SurvivalRate', 'GeneticDiversity'])
        ...
        if self.show_graph:
            ...
            self._draw_safe_zones()
            self.scatter = self.ax.scatter([], [], s=20, c='blue')
    ...
    def _draw_safe_zones(self):
        ...
        self.ax.fill(x, y, color='lightgreen', alpha=0.3)
    ...
    def update_visualization(self):
        """Update the positions of creatures in the visualization and display frame details."""
        if not self.show_graph:
            return  # Skip visualization if show_graph is False

        positions = np.array([creature.position for creature in self.population])
        colors = [creature.color for creature in self.population]
        ...
        in_safe_zone = sum(1 for c in self.population if c.is_in_safe_zone())

        # Update the information text
        self.info_text.set_text(
            f"Generation: {self.generation}\n"
            f"Frame: {self.current_time % GENERATION_LENGTH}/{GENERATION_LENGTH}\n"
...
def run_simulation(n_generations, show_graph=True):
    print(f"Starting evolution simulation for {n_generations} generations...")
    print(f"Parameters: Hidden Nodes={HIDDEN_NODES}, Population={POPULATION_SIZE}, Mutation Rate={MUTATION_RATE}")
    
    sim = Simulation(show_graph=show_graph)
    start_time = time.time()
    ...
    try:
        ...
        print(f"- {sim.csv_filename} (survival rates and genetic diversity)")
        print(f"- {sim.first_gene_pool_filename}")
        print(f"- {sim.last_gene_pool_filename}")

if __name__ == "__main__":
    import sys
    if len(sys.argv) > 1:
        n_generations = int(sys.argv[1])
        show_graph = sys.argv[2].lower() == 'true' if len(sys.argv) > 2 else True
    else:
        n_generations = 300
        show_graph = True
    
    run_simulation(n_generations, show_graph)